...
public class Main {
    ...
    public static int findLast(int[] x, int y) {
        for (int i = x.length - 1; i >= 0; i--) { // la condición de corte no contempla la posición 0 del arrelgo, por
                                                  // lo tanto se modifica a i>=0
            if (x[i] == y) {
                return i;
            }
        }
    ...
    public static void main(String[] args) {
        int[] arreglo = { 2, 2 };
        int buscar = 2;
        System.out.println(findLastE(arreglo, buscar));
    }
